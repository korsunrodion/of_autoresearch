"""
classifier_v24.py — Ordinal cascade + model risk fractions

v22: 94.59% (blend), 94.81% (best individual), 6 XGB configs
v23: 94.65% (blend), 94.74% (best LGB), blending hurt vs best individual
     → all XGB+LGB configs converge to same error profile; diversity comes from features not algos

v24 strategy:
1. Ordinal cascade: add (VH+Extreme) vs (NR+High+Low) OOF proba as 4th cascade feature
   - For isolated VH (hard FNs), ordinal model's uncertainty captures "hard-to-classify" signal
2. Model risk fraction features: model_vh_frac, model_extreme_frac, model_any_risk_frac
   - Captures "is this a risky model?" — new signal for isolated users
3. Fewer VH configs (2) with more seeds (20×) + more folds (15) — reduces blend dilution
   - v22 showed 12 seeds > 8 seeds; more seeds = lower variance on hard edge cases
"""
import os
import sys
...
def blend_results(results, y, label=""):
    """F1-weighted blend and threshold sweep."""
...
if __name__ == '__main__':
    ...
    # ── Step 1: Build cascade OOF probas ───────────────────────────────────────
    print("\n=== Step 1: Cascade OOF probas ===", flush=True)
    SEEDS_CASCADE = [42, 7, 123, 999, 2025, 1337, 2024, 101]
    ...
    print(f"  Extreme done (range: {p_extreme.min():.3f}-{p_extreme.max():.3f})", flush=True)
    ...
    # Ordinal: (VH + Extreme) vs (NR + High + Low)
    ...
    print(f"  Ordinal done (VH mean={p_ordinal[sub_ord['y'].values==1].mean():.3f}, NR mean={p_ordinal[sub_ord['y'].values==0].mean():.3f})", flush=True)
    ...
    print(f"  Augmented features: {X_vh_aug.shape[1]} (base={X_vh.shape[1]} + 4 cascade)", flush=True)
    print(f"  Cascade ordinal — VH mean={casc_ordinal[y_vh==1].mean():.3f} NR mean={casc_ordinal[y_vh==0].mean():.3f}", flush=True)

    SEEDS_VH = [42, 7, 123, 999, 2025, 1337, 2024, 101, 314, 271, 42424, 77777,
                100, 200, 300, 400, 500, 600, 700, 800]
    N_FOLDS_VH = 15

    # Best 2 XGB configs (from v22 analysis: d5-spw38, d6-spw38)
    xgb_vh_cfgs = [
        (f"XGB-d5-spw{spw_vh:.0f}",  xgb(5, 0.03, 600, spw_vh, 0.80, 0.85)),
        (f"XGB-d6-spw{spw_vh:.0f}",  xgb(6, 0.03, 500, spw_vh, 0.75, 0.80)),
    ]
    print(f"\n  XGBoost ({len(xgb_vh_cfgs)} configs × {len(SEEDS_VH)} seeds × {N_FOLDS_VH}-fold):", flush=True)
    xgb_vh_results = build_oof_xgb(X_vh_aug, y_vh, xgb_vh_cfgs, SEEDS_VH, N_FOLDS_VH)

    # Best LGB config (nl63 was best in v23)
    lgb_vh_cfgs = [
        (f"LGB-nl63-spw{spw_vh:.0f}",  lgb(63,  0.03, 700, spw_vh, 0.80, 0.80)),
        (f"LGB-nl127-spw{spw_vh:.0f}", lgb(127, 0.02, 800, spw_vh, 0.75, 0.80)),
    ]
    print(f"\n  LightGBM ({len(lgb_vh_cfgs)} configs × {len(SEEDS_VH)} seeds × {N_FOLDS_VH}-fold, CPU):", flush=True)
    lgb_vh_results = build_oof_lgb(X_vh_aug, y_vh, lgb_vh_cfgs, SEEDS_VH, N_FOLDS_VH)

    all_vh = xgb_vh_results + lgb_vh_results
    y_vh_blend, t_vh, p_vh, r_vh, f_vh = blend_results(all_vh, y_vh, "VH")

    # Also show individual bests and XGB-only blend
    xgb_blend, t_x, p_x, r_x, f_x = blend_results(xgb_vh_results, y_vh, "VH XGB-only")
    lgb_blend, t_l, p_l, r_l, f_l = blend_results(lgb_vh_results, y_vh, "VH LGB-only")
    ...
    # Report best F1 across all individual models + blends
    best_f1 = max([f for _, _, f in all_vh] + [f_vh, f_x, f_l])
    print(f"\n  Best VH F1 (any model/blend): {best_f1:.2%}", flush=True)
    ...
    print(f"\n{'='*60}")
    print(f"SUMMARY — v24 ordinal cascade + model risk fracs, selected=False")
    print(f"{'='*60}")
