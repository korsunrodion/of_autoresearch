"""
classifier_v5.py — push Very High risk F1 to 95%+

Building on v4 (92.49% F1, best ever):
- Top features were cic50k_6h, cic10k_12h, cic10k_6h (70% of importance)
- Add narrower ID ranges: 1k, 5k, 20k
- Add 8h, 10h windows to probe the 6-12h sweet spot further
- Add ID distribution features per window:
    log_id_span_Xh  = log1p(max - min of partner IDs in window)
    log_id_std_Xh   = log1p(std of partner IDs in window)
    log_min_consec_gap_Xh = min gap between consecutive sorted IDs
- Try 15-fold CV for more training data per fold
"""
import os
import sys
...
# Ranges to count close IDs
ID_RANGES = [1_000, 5_000, 10_000, 20_000, 50_000, 100_000]
RANGE_TAGS = ['1k', '5k', '10k', '20k', '50k', '100k']

# Time windows (minutes)
WINDOWS_MINUTES = [60, 60*2, 60*4, 60*6, 60*8, 60*10, 60*12, 60*24, 60*24*7, 60*24*7*2, 60*24*7*4]
...
FEATURES = (
    ...
    # ID distribution features for the key windows (6h, 12h, 24h)
    [f'log_id_span_{w}h'       for w in [6, 8, 10, 12, 24]] +
    [f'log_id_std_{w}h'        for w in [6, 8, 10, 12, 24]] +
    [f'log_min_consec_gap_{w}h' for w in [6, 8, 10, 12, 24]] +
    # model / cross-model
...
def _partner_id_stats(ids: np.ndarray):
    """Return span, std, min_consecutive_gap for a partner ID array."""
    if len(ids) == 0:
        return np.nan, np.nan, np.nan
    span = float(ids.max() - ids.min())
    std  = float(np.std(ids)) if len(ids) > 1 else 0.0
    if len(ids) >= 2:
        sorted_ids = np.sort(ids)
        min_gap = float(np.diff(sorted_ids).min())
    else:
        min_gap = np.nan
    return span, std, min_gap
...
def compute_per_user(df):
    ...
    for _, user in df.iterrows():
        ...
        for w_min, w_h in zip(WINDOWS_MINUTES, WINDOW_HOURS):
            ...
            if partners.empty:
                ...
                if w_h in KEY_WINDOWS_H:
                    span, std, mgap = _partner_id_stats(partner_ids)
                    row[f'log_id_span_{w_h}h']        = np.log1p(span)
                    row[f'log_id_std_{w_h}h']         = np.log1p(std)
                    row[f'log_min_consec_gap_{w_h}h'] = np.log1p(mgap) if not np.isnan(mgap) else np.nan
    ...
    # Ratio features
    per_user['cic_ratio_6h_24h']    = per_user['cic_50k_6h']  / (per_user['cic_50k_24h']  + 1) if 'cic_50k_6h' in per_user.columns else np.nan
    per_user['cic10k_ratio_6h_12h'] = per_user['cic_10k_6h']  / (per_user['cic_10k_12h']  + 1) if 'cic_10k_6h' in per_user.columns else np.nan
...
if __name__ == '__main__':
    ...
    print(f"SUMMARY")
    print(f"{'='*60}")
    print(f"  v4 best (finer windows + ID ranges):   92.49%")
    print(f"  This script F1:                        {fv:.2%}")
    print(f"  vs v4: {fv - 0.9249:+.2%}")
    if fv >= 0.95:
