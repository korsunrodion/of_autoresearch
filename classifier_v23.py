"""
classifier_v23.py — XGBoost + LightGBM ensemble + username entropy

v22: VH 94.59%, best individual 94.81% (gap 0.41% to 95% target).
All 6 XGB configs clustered at 94.64-94.81% — too similar to blend well.

v23 strategy:
1. LightGBM (leaf-wise tree growth) alongside XGBoost (level-wise) for real diversity
2. New features: username_digit_ratio, username_entropy
3. 3 XGB + 3 LGB configs for VH, F1-weighted blend
4. LGB uses CPU (no OpenCL GPU support available)
"""
import os
import sys
...
def build_oof_xgb(X, y, configs, seeds, n_folds):
    """XGBoost OOF. Returns list of (pool, f1)."""
    results = []
    for name, params in configs:
        ...
        results.append((pool, f))
    return results


def build_oof_lgb(X, y, configs, seeds, n_folds):
    """LightGBM OOF (CPU). Returns list of (pool, f1)."""
    results = []
    for name, params in configs:
        pool = np.zeros(len(y))
        for seed in seeds:
...
def blend_f1_weighted(pool_f1_list, y):
    """F1-weighted average of pools, then find best threshold."""
    f1s = np.array([f for _, f in pool_f1_list])
    w   = f1s / f1s.sum()
    blended = sum(wi * pool for wi, (pool, _) in zip(w, pool_f1_list))
...
if __name__ == '__main__':
    ...
    p_extreme_res = build_oof_xgb(Xe, ye, extreme_cfgs, [42, 7, 123, 999, 2025, 1337, 2024, 101], 10)
    p_extreme_all = np.mean([p for p, _ in p_extreme_res], axis=0)
    extreme_lookup = dict(zip(sub_e['user_name'].values, p_extreme_all))
    ...
    p_high_res = build_oof_xgb(Xh, yh, high_cfgs, [42, 7, 123, 999, 2025, 1337], 8)
    p_high_all = np.mean([p for p, _ in p_high_res], axis=0)
    high_lookup = dict(zip(sub_h['user_name'].values, p_high_all))
    ...
    p_low_res = build_oof_xgb(Xl, yl, low_cfgs, [42, 7, 123, 999, 2025, 1337], 8)
    p_low_all = np.mean([p for p, _ in p_low_res], axis=0)
    low_lookup = dict(zip(sub_l['user_name'].values, p_low_all))
    print("  Cascade OOF done.", flush=True)

    # ── Step 2: Very High classifier with cascade features ─────────────────────
    print("\n=== Step 2: Very High + cascade features (XGB + LGB) ===", flush=True)
    ...
    # XGBoost configs (3 best from v22)
    xgb_vh_cfgs = [
        (f"XGB-d5-spw{spw_vh:.0f}",  xgb(5, 0.03, 600, spw_vh,  0.80, 0.85)),
        (f"XGB-d6-spw{spw_vh:.0f}",  xgb(6, 0.03, 500, spw_vh,  0.75, 0.80)),
        (f"XGB-d7-spw{spw_vh:.0f}",  xgb(7, 0.02, 600, spw_vh,  0.75, 0.80)),
    ]
    print(f"\n  XGBoost VH ({len(xgb_vh_cfgs)} configs × {len(SEEDS_VH)} seeds × {N_FOLDS_VH}-fold):", flush=True)
    xgb_vh_results = build_oof_xgb(X_vh_aug, y_vh, xgb_vh_cfgs, SEEDS_VH, N_FOLDS_VH)

    # LightGBM configs (leaf-wise, fundamentally different from XGB)
    lgb_vh_cfgs = [
        (f"LGB-nl31-spw{spw_vh:.0f}",  lgb(31,  0.05, 500, spw_vh, 0.80, 0.85)),
        (f"LGB-nl63-spw{spw_vh:.0f}",  lgb(63,  0.03, 700, spw_vh, 0.80, 0.80)),
        (f"LGB-nl127-spw{spw_vh:.0f}", lgb(127, 0.02, 800, spw_vh, 0.75, 0.80)),
    ]
    print(f"\n  LightGBM VH ({len(lgb_vh_cfgs)} configs × {len(SEEDS_VH)} seeds × {N_FOLDS_VH}-fold, CPU):", flush=True)
    lgb_vh_results = build_oof_lgb(X_vh_aug, y_vh, lgb_vh_cfgs, SEEDS_VH, N_FOLDS_VH)

    # F1-weighted blend of all 6 (XGB + LGB)
    all_vh_results = xgb_vh_results + lgb_vh_results
    y_vh_blend, t_vh, p_vh, r_vh, f_vh = blend_f1_weighted(all_vh_results, y_vh)
    print(f"\n  VH XGB+LGB blend: P={p_vh:.2%} R={r_vh:.2%} F1={f_vh:.2%} @ {t_vh:.3f}", flush=True)

    # Threshold sweep
    print(f"\n  Sweep:", flush=True)
    for thr in np.arange(0.2, 0.98, 0.02):
        yt = (y_vh_blend >= thr).astype(int)
    ...
    for level in ['Extreme', 'High', 'Low']:
        ...
        res_s = build_oof_xgb(X_s, y_s, cfgs_s, [42, 7, 123, 999, 2025, 1337], 8)
        _, t_s, ps, rs, fs = blend_f1_weighted(res_s, y_s)
        print(f"  Final: P={ps:.2%} R={rs:.2%} F1={fs:.2%}", flush=True)
        results[level] = fs

    print(f"\n{'='*60}")
    print(f"SUMMARY — v23 XGB+LGB cascade, selected=False")
    print(f"{'='*60}")
