"""
classifier_v15.py — PyTorch MLP on GPU + v5 ensemble

MLPs learn smooth decision boundaries vs tree piecewise splits. With 255
positive examples the network must be small + heavily regularized, but on
GPU we can afford many seeds/folds to reduce variance.

Architecture: 178 → 256 → 128 → 64 → 1  (BatchNorm + Dropout)
Loss: weighted BCE (pos_weight = neg/pos ≈ 35)
Trained on GPU, OOF evaluation, blended with v5 HGB+XGB ensemble.
"""
import os
import sys
...
def train_mlp_oof(X_np, y_np, seeds, n_folds,
    ...
    """OOF MLP training on GPU with standardization per fold."""
    n = len(y_np)
    pos_count = y_np.sum()
...
if __name__ == '__main__':
    ...
    print(f"\n=== Tree ensemble: {len(tree_configs)} configs × {len(SEEDS)} seeds × {N_FOLDS}-fold ===", flush=True)
    tree_probas = []
    for name, needs_fill, cfg_fn in tree_configs:
    ...
    print(f"\nTree ensemble: P={pt:.2%} R={rt:.2%} F1={ft:.2%} @ {t_tree:.3f}", flush=True)

    # ── MLP on GPU ─────────────────────────────────────────────────────
    print(f"\n=== MLP (GPU) — {len(SEEDS)} seeds × {N_FOLDS}-fold ===", flush=True)
    MLP_SEEDS = SEEDS[:6]  # 6 seeds is enough for MLP (GPU is fast)
    mlp_configs = [
    ...
    for name, kw in mlp_configs:
        ...
        print(f"  {name}: P={pm:.2%} R={rm:.2%} F1={fm:.2%} @ {t_m:.3f}", flush=True)
    ...
    # ── Blend tree + MLP ───────────────────────────────────────────────
    print(f"\n=== Blend sweep (tree_w, mlp_w) ===", flush=True)
    best_blend_f1, best_alpha = 0.0, 0.0
    for alpha in np.arange(0.0, 1.05, 0.1):
        ...
        print(f"  tree={1-alpha:.1f} mlp={alpha:.1f}: P={pb:.2%} R={rb:.2%} F1={fb:.2%}{marker}", flush=True)
        if fb > best_blend_f1:
            best_blend_f1, best_alpha = fb, alpha

    y_best = (1 - best_alpha) * y_proba_tree + best_alpha * y_proba_mlp
    t_best = find_best_threshold(y, y_best)

    # Hybrid rule on best blend
    print(f"\nHybrid rule (elsewhere>=4) on best blend (alpha={best_alpha:.1f}):")
    for low_t in [0.25, 0.30, 0.35, 0.40]:
        yh = np.where(elsewhere_mask & (y_best >= low_t), 1, (y_best >= t_best).astype(int))
    ...
    print(f"  Tree ensemble:    {ft:.2%}")
    print(f"  MLP ensemble:     {fm:.2%}")
    print(f"  Best blend F1:    {best_blend_f1:.2%}  (tree={1-best_alpha:.1f} mlp={best_alpha:.1f})")
