"""
classifier_v8.py — v5 + 7h window

v5 achieved 93.28% F1. v6b showed cic_10k_7h was the top feature at 37.6%
when 7h was included. Adding 7h between the existing 6h and 8h windows to
capture borderline false negatives that register within 7 hours.
"""
import os
import sys
...
WINDOWS_MINUTES = [60, 60*2, 60*4, 60*6, 60*7, 60*8, 60*10, 60*12,
                   60*24, 60*24*7, 60*24*7*2, 60*24*7*4]
WINDOW_HOURS    = [w // 60 for w in WINDOWS_MINUTES]  # 1,2,4,6,7,8,10,12,24,168,336,672

KEY_WINDOWS_H = {6, 7, 8, 10, 12, 24}
            ...
            if partners.empty:
                ...
                if w_h in KEY_WINDOWS_H:
                    if len(partner_ids) > 0:
                        span = float(partner_ids.max() - partner_ids.min())
                        std  = float(np.std(partner_ids)) if len(partner_ids) > 1 else 0.0
...
if __name__ == '__main__':
    ...
    print(f"\n=== v8 Ensemble: {len(configs)} configs × {len(SEEDS)} seeds × {N_FOLDS}-fold OOF ===", flush=True)
    all_probas = []
    ...
    print(f"\n{'='*60}")
    print(f"v8 Ensemble ({len(all_probas)} configs, opt threshold={best_thresh:.3f}):")
