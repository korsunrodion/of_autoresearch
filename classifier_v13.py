"""
classifier_v13.py — v5 + binary elsewhere flags + 3h/5h windows

Changes from v5:
1. Add binary flags: is_vh_elsewhere (risk_elsewhere >= 4),
   is_extreme_elsewhere (risk_elsewhere >= 5). The existing user_max_risk_elsewhere
   feature is 1-5 linear; tree must discover >= 4 matters. A binary flag makes it
   trivial to split on, helping the 4 FN bots known to be VH elsewhere.

2. Add 3h and 5h windows to fill the 2h–6h gap. Might capture bots that
   register in 3-5h bursts (between the 2h and 6h windows we already have).
"""
import os
import sys
...
WINDOWS_MINUTES = [60, 60*2, 60*3, 60*4, 60*5, 60*6, 60*8, 60*10, 60*12,
                   60*24, 60*24*7, 60*24*7*2, 60*24*7*4]
WINDOW_HOURS    = [w // 60 for w in WINDOWS_MINUTES]  # 1,2,3,4,5,6,8,10,12,24,168,336,672
...
FEATURES = (
    ...
    ['log_min_time_diff', 'log_model_norisk_median_id_diff', 'rel_min_id_diff_24h',
     ...
     # NEW: binary elsewhere flags
     'is_vh_elsewhere', 'is_extreme_elsewhere']
)
...
def compute_per_user(df):
    ...
    # Binary elsewhere flags
    per_user['is_vh_elsewhere']      = (per_user['user_max_risk_elsewhere'] >= 4).astype(float)
    per_user['is_extreme_elsewhere'] = (per_user['user_max_risk_elsewhere'] >= 5).astype(float)
    return per_user
...
if __name__ == '__main__':
    ...
    print(f"\n=== v13 Ensemble: {len(configs)} configs × {len(SEEDS)} seeds × {N_FOLDS}-fold OOF ===", flush=True)
    all_probas = []
    ...
    print(f"\n{'='*60}")
    print(f"v13 Ensemble ({len(all_probas)} configs, opt threshold={best_thresh:.3f}):")
    print(f"  Precision: {pv:.2%}  Recall: {rv:.2%}  F1: {fv:.2%}")

    # Check elsewhere flag impact
    elsewhere_mask = subset['user_max_risk_elsewhere'].values >= 4
    vh_mask = y == 1
    print(f"\n  VH with elsewhere>=4: {(vh_mask & elsewhere_mask).sum()} users")
    print(f"  No risk with elsewhere>=4: {(~vh_mask & elsewhere_mask).sum()} users")

    # Show VH-elsewhere users: score vs classification
    print(f"\n  VH+elsewhere>=4 scores (sorted):")
    vh_elsewhere_mask = vh_mask & elsewhere_mask
    scores_ve = y_proba[vh_elsewhere_mask]
    names_ve = subset[vh_mask & elsewhere_mask]['user_name'].values
    for nm, sc in sorted(zip(names_ve, scores_ve), key=lambda x: x[1]):
        status = "TP" if sc >= best_thresh else "FN"
        print(f"    {nm:<30} {sc:.3f} [{status}]")
    ...
    # Hybrid rule: lower threshold for users with high elsewhere risk
    print(f"\nHybrid threshold rule tests:")
    for low_t in [0.20, 0.25, 0.30, 0.35, 0.40]:
        y_hybrid = np.where(
