"""
classifier_v1_cpu.py — CPU-only, no GPU/PyTorch dependency

Key changes from v25:
- Numpy vectorized pairwise ops (replaces PyTorch CUDA tensors)
- LightGBM for ALL models (cascade + VH); XGBoost removed
- multiprocessing.Pool for per-model feature parallelism
- No CUDA/OpenCL required

Target: VH F1 >= 95%, ~20-40 min on 8-core CPU (no GPU needed)
...
if __name__ == '__main__':
    ...
    # Cascade configs: lighter LGB (nl63) for speed; accuracy of cascade
    # doesn't need to be perfect, just informative for the VH augmentation.
    ...
    res_e  = build_oof_lgb(Xe, ye, [
        (f'E-nl63-spw{spw_e:.0f}',            lgb_c(63, spw_e)),
        (f'E-nl63-spw{min(spw_e*2,200):.0f}', lgb_c(63, min(spw_e * 2, 200))),
    ], SEEDS_C, N_FOLDS_C)
    ...
    res_h  = build_oof_lgb(Xh, yh, [(f'H-nl63-spw{spw_h:.0f}', lgb_c(63, spw_h))],
                           SEEDS_C[:6], N_FOLDS_C)
    ...
    res_l  = build_oof_lgb(Xl, yl, [(f'L-nl63-spw{spw_l:.0f}', lgb_c(63, spw_l))],
                           SEEDS_C[:6], N_FOLDS_C)
    ...
    res_ord = build_oof_lgb(X_ord, y_ord,
                            [(f'ORD-nl63-spw{spw_ord:.0f}', lgb_c(63, spw_ord))],
                            SEEDS_C[:6], N_FOLDS_C)
    ...
    vh_results = build_oof_lgb(X_vh_aug, y_vh, [
        ...
        (f'LGB-nl255-spw{spw_vh:.0f}',
         dict(num_leaves=255, learning_rate=0.01, n_estimators=1000,
              scale_pos_weight=spw_vh, subsample=0.75, colsample_bytree=0.80,
              min_child_samples=10, subsample_freq=1, verbose=-1, n_jobs=-1)),
    ], SEEDS_VH, N_FOLDS_VH)
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print(f"SUMMARY — v1_cpu (LGB-only, numpy, multiprocessing)")
    print(f"{'='*60}")
