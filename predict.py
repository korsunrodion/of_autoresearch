"""
classifier_cpu_predict.py — CPU-only production train/predict script

Usage:
  # Train on full dataset and save models:
  python classifier_cpu_predict.py --train [--model-dir models_cpu]

  # Predict for all users (or a subset):
  python classifier_cpu_predict.py --predict [--users user1 user2] [--output results.csv]
...
JS export (optional):
  pip install m2cgen
  python classifier_cpu_predict.py --train --export-js vh_lgb.js
"""
import os
import sys
import argparse
...
def predict(model_dir, users=None, output=None):
    ...
    if 'is_internal_data' in df.columns:
        ...
        # implicit cold (pure no-risk models that happen to have model_any_risk_frac=0).
    ...
    #   1. Explicit cold  — model has is_internal_data=False rows (real new-batch subscription)
    #   2. Warm           — model_any_risk_frac > 0 (established labeled history)
    #   3. Implicit cold  — model_any_risk_frac=0 but only warm rows (pure no-risk model)
    # This prevents pure no-risk production models (Extreme-heavy in cold cascade)
    # from overriding correct VH/High predictions on the real cold batch.
    ...
    # Warm: include all users not covered by explicit cold
    ...
    # Implicit cold: only for users not covered by explicit cold or warm
