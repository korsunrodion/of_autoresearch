"""
classifier_v14.py — v5 core + aggressive high-recall XGB configs + hybrid rule

Current bottleneck: 21 FN bots. Need to reduce to ~13.
- 4 FN bots are "known elsewhere" (score 0.026-0.590, most need lower threshold)
- ~12 FN bots are locally ambiguous

Strategy:
1. Add XGB configs with much higher scale_pos_weight (20, 35, 70) to push recall
2. Test a hybrid rule: lower threshold for users with user_max_risk_elsewhere >= 4
3. Report F1 at various threshold/hybrid combinations

The ensemble mean of high-recall configs (spw 20-70) with standard configs (spw 3-10)
might better calibrate scores for borderline VH users.
"""
import os
import sys
...
if __name__ == '__main__':
    ...
    # More seeds + folds now that XGB runs on GPU (much faster)
    SEEDS   = [42, 7, 123, 999, 2025, 1337, 2024, 101, 314, 271, 1776, 2023]
    N_FOLDS = 20

    XGB_KW = dict(n_estimators=600, max_depth=4, learning_rate=0.04,
                  subsample=0.85, colsample_bytree=0.85,
                  eval_metric='logloss', verbosity=0,
                  device='cuda', random_state=0)

    configs = [
        # HistGB (CPU, NaN-native)
        ...
        ("HGB-w70",   False, lambda s: HistGradientBoostingClassifier(max_iter=800,  max_depth=4, learning_rate=0.02, min_samples_leaf=8, l2_regularization=0.5, class_weight={0: 1, 1: 70}, random_state=s)),
        # XGB on GPU — much faster, so we use more trees (600) and diverse weights
        ("XGB-spw3",  True,  lambda s: XGBClassifier(**{**XGB_KW, 'scale_pos_weight': 3.0,  'random_state': s})),
        ("XGB-spw5",  True,  lambda s: XGBClassifier(**{**XGB_KW, 'scale_pos_weight': 5.0,  'random_state': s})),
        ("XGB-spw10", True,  lambda s: XGBClassifier(**{**XGB_KW, 'scale_pos_weight': 10.0, 'random_state': s})),
        ("XGB-spw20", True,  lambda s: XGBClassifier(**{**XGB_KW, 'scale_pos_weight': 20.0, 'random_state': s})),
        ("XGB-spw35", True,  lambda s: XGBClassifier(**{**XGB_KW, 'scale_pos_weight': 35.0, 'random_state': s})),
        ("XGB-d3",    True,  lambda s: XGBClassifier(**{**XGB_KW, 'scale_pos_weight': 5.0,  'max_depth': 3, 'random_state': s})),
        ("XGB-d6",    True,  lambda s: XGBClassifier(**{**XGB_KW, 'scale_pos_weight': 5.0,  'max_depth': 6, 'random_state': s})),
    ]

    print(f"\n=== v14 Ensemble: {len(configs)} configs × {len(SEEDS)} seeds × {N_FOLDS}-fold OOF ===", flush=True)
    all_probas = []
    ...
    # ── v5 core (first 10 configs) ──
    y_proba_v5 = np.mean(all_probas[:10], axis=0)
    best_thresh_v5 = find_best_threshold(y, y_proba_v5)
    y_pred_v5 = (y_proba_v5 >= best_thresh_v5).astype(int)
    pv5, rv5, fv5, _ = precision_recall_fscore_support(y, y_pred_v5, pos_label=1, average='binary', zero_division=0)
    print(f"\n  v5 core (10 configs): P={pv5:.2%} R={rv5:.2%} F1={fv5:.2%} @ {best_thresh_v5:.3f}")
    ...
    print(f"\n{'='*60}")
    print(f"v14 Full Ensemble ({len(configs)} configs, opt threshold={best_thresh:.3f}):")
    print(f"  Precision: {pv:.2%}  Recall: {rv:.2%}  F1: {fv:.2%}")

    # ── Hybrid rule analysis on v5 probabilities ──
    print(f"\nFN bots with elsewhere>=4 (using v5 probabilities):")
    vh_mask = y == 1
    fn_elsewhere = vh_mask & elsewhere_mask & (y_proba_v5 < best_thresh_v5)
    fn_names = subset[fn_elsewhere]['user_name'].values
    fn_scores = y_proba_v5[fn_elsewhere]
    for nm, sc in sorted(zip(fn_names, fn_scores), key=lambda x: x[1]):
        print(f"  {nm:<30} {sc:.3f} [FN]")

    print(f"\nHybrid rule on v5 probas (lower threshold for elsewhere>=4):")
    for low_t in [0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45]:
        y_hybrid = np.where(
            elsewhere_mask & (y_proba_v5 >= low_t),
            1,
            (y_proba_v5 >= best_thresh_v5).astype(int)
        )
        ph, rh, fh, _ = precision_recall_fscore_support(y, y_hybrid, pos_label=1, average='binary', zero_division=0)
        tp = y_hybrid[vh_mask].sum()
        fp = y_hybrid[~vh_mask].sum() - (y_pred_v5[~vh_mask]).sum()
        print(f"  elsewhere>=4 → thresh={low_t:.2f}: P={ph:.2%} R={rh:.2%} F1={fh:.2%}  TP={tp} (+{tp-(y_pred_v5[vh_mask]).sum()}) FP_extra≈{fp}")

    print(f"\nHybrid rule on FULL ensemble probas:")
    for low_t in [0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45]:
        y_hybrid = np.where(
            elsewhere_mask & (y_proba >= low_t),
            1,
            (y_proba >= best_thresh).astype(int)
        )
        ph, rh, fh, _ = precision_recall_fscore_support(y, y_hybrid, pos_label=1, average='binary', zero_division=0)
        print(f"  elsewhere>=4 → thresh={low_t:.2f}: P={ph:.2%} R={rh:.2%} F1={fh:.2%}")

    print(f"\nThreshold sweep (full ensemble):")
