"""
classifier_v9.py — v5 + gap distribution + shell features

Type-3 false negatives have:
  - High partner_count_8h (~13.6) but low cic_10k_8h (~2.67)
  - High log_min_id_diff_8h (~8.28 vs TP 5.37)

They register with many contemporaries but spread-out IDs. The minimum
consecutive gap (already in v5) may not discriminate if gaps are large
but REGULAR (arithmetic sequence). New features:
  - log_median_consec_gap_Xh / log_mean_consec_gap_Xh: captures typical spacing
  - log_cv_consec_gap_Xh: low CV = regular gaps = more bot-like
  - shell_Xh = cic_50k - cic_10k: partners in "second shell" (10k-50k range)
  - far_count_Xh = partner_count - cic_100k: partners beyond 100k (distinct range)
  - log_id_span_per_partner_Xh: average ID spread per partner
"""
import os
import sys
...
WINDOW_HOURS    = [w // 60 for w in WINDOWS_MINUTES]  # 1,2,4,6,8,10,12,24,168,336,672
...
FEATURES = (
    ...
    # ID distribution features for key windows
    ...
    # NEW: gap distribution features
    [f'log_median_consec_gap_{w}h' for w in sorted(KEY_WINDOWS_H)] +
    [f'log_mean_consec_gap_{w}h'   for w in sorted(KEY_WINDOWS_H)] +
    [f'log_cv_consec_gap_{w}h'     for w in sorted(KEY_WINDOWS_H)] +
    # NEW: shell features (ID range bands)
    [f'shell_10k50k_{w}h'  for w in sorted(KEY_WINDOWS_H)] +
    [f'shell_50k100k_{w}h' for w in sorted(KEY_WINDOWS_H)] +
    [f'far_count_{w}h'     for w in sorted(KEY_WINDOWS_H)] +
    # NEW: span per partner
    [f'log_span_per_partner_{w}h' for w in sorted(KEY_WINDOWS_H)] +
    # scalar / cross-model
...
def _gap_stats(sorted_ids):
    """Return (min, median, mean, cv) of consecutive gaps for sorted ID array."""
    if len(sorted_ids) < 2:
        return np.nan, np.nan, np.nan, np.nan
    gaps = np.diff(sorted_ids).astype(float)
    g_min    = gaps.min()
    g_median = np.median(gaps)
    g_mean   = gaps.mean()
    g_cv     = gaps.std() / (g_mean + 1.0)
    return g_min, g_median, g_mean, g_cv
        ...
        for w_min, w_h in zip(WINDOWS_MINUTES, WINDOW_HOURS):
            ...
            if partners.empty:
                ...
                if w_h in KEY_WINDOWS_H:
                    for feat in ['log_id_span', 'log_id_std', 'log_min_consec_gap',
                                 'log_median_consec_gap', 'log_mean_consec_gap',
                                 'log_cv_consec_gap', 'shell_10k50k', 'shell_50k100k',
                                 'far_count', 'log_span_per_partner']:
                        row[f'{feat}_{w_h}h'] = np.nan
                any_empty = True
            else:
                ...
                if w_h in KEY_WINDOWS_H:
                    ...
                    sorted_ids = np.sort(partner_ids)
                    g_min, g_median, g_mean, g_cv = _gap_stats(sorted_ids)
                    row[f'log_min_consec_gap_{w_h}h']    = np.log1p(g_min)    if not np.isnan(g_min)    else np.nan
                    row[f'log_median_consec_gap_{w_h}h'] = np.log1p(g_median) if not np.isnan(g_median) else np.nan
                    row[f'log_mean_consec_gap_{w_h}h']   = np.log1p(g_mean)   if not np.isnan(g_mean)   else np.nan
                    row[f'log_cv_consec_gap_{w_h}h']     = np.log1p(g_cv)     if not np.isnan(g_cv)     else np.nan

                    cic_10k  = float((id_diffs <= 10_000).sum())
                    cic_50k  = float((id_diffs <= 50_000).sum())
                    cic_100k = float((id_diffs <= 100_000).sum())
                    row[f'shell_10k50k_{w_h}h']  = cic_50k  - cic_10k
                    row[f'shell_50k100k_{w_h}h'] = cic_100k - cic_50k
                    row[f'far_count_{w_h}h']      = float(pc) - cic_100k
                    row[f'log_span_per_partner_{w_h}h'] = np.log1p(span / (pc + 1))
...
if __name__ == '__main__':
    ...
    print(f"\n=== v9 Ensemble: {len(configs)} configs × {len(SEEDS)} seeds × {N_FOLDS}-fold OOF ===", flush=True)
    all_probas = []
    ...
    print(f"\n{'='*60}")
    print(f"v9 Ensemble ({len(all_probas)} configs, opt threshold={best_thresh:.3f}):")
